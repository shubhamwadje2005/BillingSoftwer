...
const CustomergeneratePdf = (bill, res) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.text(bill.customerName ? bill.customerName.charAt(0).toUpperCase() + bill.customerName.slice(1) : "N/A", 60, currentY + 35);
            doc.text(bill.customerPhone || "N/A", 420, currentY + 35);

            currentY += 90;

            /* ================= ITEMS SECTION ================= */
            const itemBoxHeight = ((bill.items && bill.items.length ? bill.items.length : 0) * 65) + 40;
            ...
            if (bill.items && bill.items.length) {
                bill.items.forEach((item, index) => {
                    ...
                    doc.text(item.productName || "Item", 60, itemY);
                    doc.text(`Rs ${item.price || 0}`, 480, itemY, { width: 50, align: "right" });
                    ...
                    doc.text(`Total: Rs ${itemTotal || 0}`, 480, itemY + 18, { width: 50, align: "right" });
            ...
            renderRow("Total", `Rs ${bill.totalAmount || 0}`, totalY + 90, "#000", true);

            /* ================= PAYMENT STATUS ================= */
            let footerY = currentY + 160;

            doc.fillColor("#6b7280").fontSize(10).font("Helvetica").text("Payment Method", 60, footerY);
            doc.fillColor("#000").fontSize(12).font("Helvetica-Bold").text(bill.paymentMethod || "N/A", 60, footerY + 18);
